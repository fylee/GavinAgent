# 028 — Refactor Agent Loop: assemble_context + call_llm
...
- [ ] All existing unit tests in `tests/agent/` pass（無迴歸）
- [ ] `tests/agent/test_nodes_refactor.py` 的所有新 test cases 通過
...
測試檔：`tests/agent/test_nodes_refactor.py`
風格：pytest + `unittest.mock.patch`，不起真實 LLM 或 DB 呼叫。
DB 相關測試用 `@pytest.mark.django_db`。
...
class TestIsCancelled:
    ...
    def test_failed_run_returns_true(self):
        """AgentRun 狀態為 FAILED → True"""
        from agent.models import AgentRun
        from agent.graph.nodes import _is_cancelled
        run = AgentRun.objects.create(status=AgentRun.Status.FAILED, input="x")
        assert _is_cancelled(str(run.pk)) is True

    @pytest.mark.django_db
    def test_running_run_returns_false(self):
        """AgentRun 狀態為 RUNNING → False"""
        from agent.models import AgentRun
        from agent.graph.nodes import _is_cancelled
        run = AgentRun.objects.create(status=AgentRun.Status.RUNNING, input="x")
        assert _is_cancelled(str(run.pk)) is False

    def test_db_error_returns_false(self):
        """DB 查詢失敗時不崩潰，回傳 False"""
        from agent.graph.nodes import _is_cancelled
...
class TestAssembleMessages:
    ...
    def test_no_conversation_appends_user_message(self):
        """conversation_id=None → 直接把 input 當 user message"""
    ...
    def test_error_prefix_messages_filtered(self, settings):
        """assistant 訊息以 error prefix 開頭時，從 history 中移除"""
        from agent.graph.nodes import _assemble_messages
        from unittest.mock import patch

        chat_msgs = [
            {"role": "user", "content": "hi"},
            {"role": "assistant", "content": "LLM error: timeout"},   # 應被過濾
    ...
    def test_history_window_truncation(self, settings):
        """AGENT_HISTORY_WINDOW=2 → 只保留最後 2 筆"""
        from agent.graph.nodes import _assemble_messages
    ...
    def test_tool_results_injected_when_ids_match(self):
        """required_ids ⊆ result_ids → assistant_tool_call_message + tool results 注入"""
        from agent.graph.nodes import _assemble_messages
    ...
    def test_tool_results_skipped_when_ids_missing(self):
        """result_ids 不完整 → assistant message 與 tool results 都不注入"""
        from agent.graph.nodes import _assemble_messages
        ...
        tool_results = [{"tool_call_id": "tc-1", "result": {}}]  # tc-2 缺少
        state = self._base_state(
            assistant_tool_call_message=assistant_msg,
            tool_results=tool_results,
        )
    ...
    def test_collected_markdown_appended_concluding_round(self):
        """collected_markdown 有內容且無 tool_results → 注入 verbatim 提示"""
        from agent.graph.nodes import _assemble_messages
    ...
    def test_history_stats_returned(self, settings):
        """回傳的 stats dict 包含 history_messages 與 history_dropped"""
        from agent.graph.nodes import _assemble_messages
...
class TestPersistLoopTrace:
    ...
    def test_writes_loop_trace_to_graph_state(self):
        """loop_trace 寫入 AgentRun.graph_state['loop_trace']"""
        from agent.models import AgentRun
        from agent.graph.nodes import _persist_loop_trace

        run = AgentRun.objects.create(input="x", graph_state={})
        trace = [{"round": 1, "decision": "tool_calls"}]
    ...
    def test_nonexistent_run_does_not_raise(self):
        """run_id 不存在時不拋例外"""
        from agent.graph.nodes import _persist_loop_trace
        _persist_loop_trace("nonexistent-99999", [{"round": 1}])  # 不應 raise
...
class TestAssembleContext:
    def test_returns_all_seven_state_fields(self):
        """assemble_context 回傳 dict 包含所有 7 個 _* 欄位"""
    ...
    def test_conversation_id_appended_to_system_content(self):
        """conversation_id が存在する場合、_system_content に含まれる"""
        from agent.graph.nodes import assemble_context
    ...
    def test_no_conversation_id_system_content_unchanged(self):
        """conversation_id=None のとき _system_content はそのまま"""
        from agent.graph.nodes import assemble_context
...
class TestCallLlmResumptionFallback:
    def test_empty_system_content_triggers_rebuild(self):
        """
        tool-approval 恢復時 _system_content 可能為空（舊 state 無此欄位）。
        call_llm 應 fallback 到 _build_system_context() 重新建立，不應回傳空訊息。
        """
        ...
        # _build_system_context 被呼叫表示有 fallback
